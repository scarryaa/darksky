...
import { type AppBskyEmbedImages, AppBskyFeedDefs, AppBskyFeedPost, AtUri, RichText } from '@atproto/api';
...
const Post = ({ post, reason, style }: PostProps): JSX.Element => {
  ...
  const postLink = `/profile/${post.author.did}/post/${post.uri.split('/')[4]}`;
              ...
              <Link hoverUnderline={false} link={`/profile/${post.author.did}`}>
                ...
                <View style={styles.postContent}>
                    <Text>
                        ...
                        &nbsp;
                        {/* TODO This errors if made into an actual link. */}
                        {/* TODO add tooltip for the date */}
                        <Link hoverUnderline={true} link={null} style={[theme.typography.sm, { color: theme.colors.textGrey }]}>{ago(post.indexedAt)}</Link>
                    </Text>
                    ...
                    {(post.embed?.images != null) &&
                      <PostImages images={post.embed.images as AppBskyEmbedImages.ViewImage[]} />}
