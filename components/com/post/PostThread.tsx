...
const PostThread = ({ post }: PostThreadProps): JSX.Element => {
  ...
  useEffect(() => {
    if (!loading && (currentPostRef.current != null)) {
  ...
  return (
        <View>
            ...
            <div ref={currentPostRef}>
                {(currentPost != null) && <PostDetail style={{ borderTopWidth: filteredPosts.length > 0 ? 1 : 0 }} post={currentPost} />}
            </div>
